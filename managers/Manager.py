...
class Manager:
    ...
    def get_timepoints(self, timetable):
        ...
        if timetable['mode'] == 'daily':
            points = list(
                map(lambda p: datetime(minute=int(p['minute']), hour=int(p['hour']), day=now.day, month=now.month,
                                       year=now.year), raw))
            points.sort()
            if points:
        ...
        if timetable['mode'] == 'monthly':
            points = list(
                map(lambda p: datetime(minute=int(p['minute']), hour=int(p['hour']), day=int(p['day']), month=now.month,
                                       year=now.year), raw))
            points.sort()
            if points:
                points.append(
                    datetime(minute=points[0].minute, hour=points[0].hour, day=points[0].day, month=points[0].month + 1,
                             year=now.year))
        return points
