    ...
    <div v-if="medicines">
        ...
        <div class="row">
            <card v-for="(medicine, i) in medicines" :key="medicine.id" :image="images.medicine" class="col-lg-6 col-md-7">
                ...
                <a href="#" v-if="!medicine.notifications_disabled" @click="disable_notifications(medicine)">Отключить уведомления</a>
                <a href="#" v-if="medicine.notifications_disabled" @click="enable_notifications(medicine)">Включить уведомления</a>
            </card>
        </div>
    </div>
</template>
